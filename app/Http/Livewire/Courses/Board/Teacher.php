...
class Teacher extends Component
{
    ...
    public function mount(Int $teacher_id = 0, String $origin = null)
    {
        $this->course = ReaCourse::Where('course_name', 'rea')->first();
        if ($origin == NULL) {
            $this->board = route('course.panel.home');
        } else if ($origin == "board") {
            $this->board = route('course.board.home');
        } else if ($origin == "evaluation") {
            $this->board = route('repositories.index');
        }
        $id = ($teacher_id == 0 ? auth()->user()->id : $teacher_id);
        $teacher = User::Where(function ($query) {
            $query->whereHas('reaCourseResponses')->orWhereHas('reaCourseEvidences');
        })->where('id', $id)->first();
        ...
        if ($teacher) {
            ...
            $this->user_progress();
            $this->getCourseDays($this->teacher);
        } else {
    ...
    {
        return view('livewire.courses.board.teacher');
    }
